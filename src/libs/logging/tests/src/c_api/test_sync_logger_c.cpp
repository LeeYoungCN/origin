...
class TestSyncLoggerC : public ::testing::Test {
protected:
...
TEST_F(TestSyncLoggerC, create_single_sink)
{
...
TEST_F(TestSyncLoggerC, create_initializer_list)
{
    const std::string name = get_logger_name(test_info_);
...
TEST_F(TestSyncLoggerC, create_vector)
{
    const std::string name = get_logger_name(test_info_);
...
TEST_F(TestSyncLoggerC, log_level)
{
    const std::string name = get_logger_name(test_info_);
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, flush_level)
{
    const std::string name = get_logger_name(test_info_);
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, log_log)
{
    const std::string name = get_logger_name(test_info_);
...
TEST_F(TestSyncLoggerC, log_flush)
{
    const std::string name = get_logger_name(test_info_);
    _sink->set_level(LOG_LEVELS.at(0));
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, log_flush_on)
{
    const std::string name = get_logger_name(test_info_);
    _sink->set_level(LOG_LEVELS.at(0));
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, log_function)
{
    const std::string name = get_logger_name(test_info_);
    _sink->set_level(LOG_LEVELS.at(0));
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, set_pattern)
{
    const std::string name = get_logger_name(test_info_);
    _logger = std::make_shared<SyncLogger>(name, _sink);
...
TEST_F(TestSyncLoggerC, set_formatter)
{
    const std::string name = get_logger_name(test_info_);
    _sink->set_level(LOG_LEVELS.at(0));
    _logger = std::make_shared<SyncLogger>(name, _sink);
